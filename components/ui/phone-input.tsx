      ...
      <ComboboxTrigger
        render={
          <Button
            ...
            size={variant}
            className={cn(
              "rounded-s-lg h-12 flex gap-1 w-full px-4 py-3 leading-none hover:bg-transparent focus:z-10 data-pressed:bg-transparent",
              disabled && "opacity-50"
            )}
            disabled={disabled}
          >
...
function FlagComponent({ country, countryName }: BasePhoneInput.FlagProps) {
  ...
  return (
    <span className="flex size-8 items-center justify-center [&_svg:not([class*='size-'])]:size-full! [&_svg:not([class*='size-'])]:rounded-[5px]">
      {Flag ? (
